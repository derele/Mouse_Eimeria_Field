...
## Choice of threshold : (cf papers Enas & Victor)
## 5 if we are being more sensitive; 
## 3.75 if we are more specific  

prepareQpcr <- function(THRESHOLD){
  library(dplyr)
  ...
  myqPCRformat <- function(df){
    ...
    df$qPCRsummary[df$delta_ct_cewe_MminusE <= -THRESHOLD & df$delta_ct_ilwe_MminusE <= -THRESHOLD] <- "non infected"
    df$qPCRsummary[df$delta_ct_cewe_MminusE > -THRESHOLD & df$delta_ct_ilwe_MminusE <= -THRESHOLD] <- "infected cecum"
    df$qPCRsummary[df$delta_ct_cewe_MminusE <= -THRESHOLD & df$delta_ct_ilwe_MminusE > -THRESHOLD] <- "infected ileum"
    df$qPCRsummary[df$delta_ct_cewe_MminusE > -THRESHOLD & df$delta_ct_ilwe_MminusE > -THRESHOLD &
    ...
    df$qPCRsummary[df$delta_ct_cewe_MminusE > -THRESHOLD & is.na(df$delta_ct_ilwe_MminusE)] <- "infected cecum [ileum.NA]"
    df$qPCRsummary[is.na(df$delta_ct_cewe_MminusE) & df$delta_ct_ilwe_MminusE > -THRESHOLD] <- "infected ileum [cecum.NA]"
    df$qPCRsummary[df$delta_ct_cewe_MminusE <= -THRESHOLD & is.na(df$delta_ct_ilwe_MminusE)] <- "non infected cecum [ileum.NA]"
    df$qPCRsummary[is.na(df$delta_ct_cewe_MminusE) & df$delta_ct_ilwe_MminusE <= -THRESHOLD] <- "non infected ileum [cecum.NA]"
    ...
    df$delta_ct_MminusE[is.na(df$delta_ct_MminusE)] <- -THRESHOLD
  ...
  ggplot(qpcrData_2016_2017, aes(x = Mouse_ID, y = delta_ct_MminusE,
    ...
    facet_grid(.~year) +
    theme_bw()
  return(qpcrData_2016_2017)
}

qpcrData_2016_2017_threshold5 <- prepareQpcr(5)
