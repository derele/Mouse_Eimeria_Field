...
Number of Mus musculus caught with OPG values: `r nrow(myData[!is.na(myData$OPG),])`
...
PCR positive = one of the 3 markers 18S, COI or ORF470) gave a sequence. Number of Mus musculus caught with PCR performed: `r nrow(myData[!is.na(myData$PCRstatus),])`
...
```{r, echo = FALSE}
write.csv(myData, paste0("../FinalFullDF_flotationPcrqPCR_Threshold_", THRESHOLD, ".csv"), row.names = F)
```

# PART II for comparison: qPCR detection threshold = -3.75

```{r, echo = FALSE}
# Define threshold for qPCR data (cf Enas & Victor papers)
THRESHOLD <- 3.75 # choice of specificity
...
```{r oocystsDetec_3.75, echo=FALSE, warning=FALSE}
flotDF$OPG <- as.numeric(as.character(flotDF$OPG))
flotDF <- flotDF[!is.na(flotDF$OPG),]
...
```{r oocystssmooth_3.75, echo=FALSE, warning=FALSE}
ggplot(myData[!is.na(myData$OPG) &
                               myData$OPG >0,], aes(x = HI, y = OPG+1)) +
  geom_point(aes(fill = as.factor(Year)), pch = 21, alpha = .8, size = 4) +
  geom_smooth(se=F) +
...
```{r pcr_3.75, echo=FALSE, warning=FALSE}
#correct wrong names
toremove <- paste0(paste0("X",1:30, "_"), collapse = "|")
names(PCRdf) <- gsub(toremove, "", names(PCRdf))
names(PCRdf)[names(PCRdf)%in%"ID_mouse"] <- "Mouse_ID"
...
```{r generalstats_3.75, echo = FALSE, message=FALSE}
## Remove useless mice:
# wildpark Schorfheide (not needed, test)
wsh <- c(paste0("AA_000", 1:9), paste0("AA_00", 10:46))
# apodemus caught in 2016
...
```{r plotDensHI_3.75, echo=FALSE, warning=FALSE, fig.width=5,fig.height=3,fig.cap="\\label{fig:plot1}Number of animals caught along the hybrid index "}
ggplot(myData, aes(x = HI)) +
  geom_histogram(binwidth = 0.05, col = "black", fill = "lightblue") +
  theme_bw() +
  ggtitle("Density of hybrids") +
...
```{r prevcompared_3.75, echo = FALSE}
prevalenceFlotation <- getPrevalenceTable(table(myData$OPG > 0, 
                                                myData$Year))
prevalencePCR <- getPrevalenceTable(table(myData$PCRstatus, 
                                          myData$Year))
...
```{r tabPrevOO_3.75, echo=FALSE, results='asis'}
library(knitr)
kable(prevalenceFlotation, caption = "Prevalence of Eimeria per year, based on oocyst flotation")
```

```{r tabPrevPCR_3.75, echo=FALSE, results='asis'}
library(knitr)
kable(prevalencePCR, caption = "Prevalence of Eimeria per year, based on PCR detection. A mouse was considered infected by Eimeria ifone of the 3 markers (COI, 18S or ORF470) gave a sequence")
```

```{r tabPrevqPCR_3.75, echo=FALSE, results='asis'}
library(knitr)
kable(prevalenceqPCR, caption = "Prevalence of Eimeria per year, based on qPCR")
```

```{r tabPrevtot_3.75, echo=FALSE, results='asis'}
kable(prevalenceTot, caption = "Prevalence of Eimeria per year, based on all detections methods. A mouse was considered infected by Eimeria if one of the 3 markers (COI, 18S or ORF470) gave a sequence, OR if it had a positive count of oocysts in its feces, OR if it was qPCR positive")
```

## OPG-PCR

```{r opgpcr_3.75, echo=FALSE, warning=FALSE}
# first, compare PCR and oocysts
completeData1 <- myData[!is.na(myData$OPG) &
                          !is.na(myData$PCRstatus),]
...
myVennDiagram2 <- function(data){      
  ...
  g <- draw.pairwise.venn(area1 = area1, 
                          ...
                          cat.col = c("grey","green"),
                          cat.fontfamily = "serif")
  grid.arrange(gTree(children=g), bottom="3 methods, threshold for qPCR = - 5")
}
...
```{r opgpcrVenn_3.75, echo=FALSE, warning=FALSE}
# first, compare PCR and oocysts
completeDataopgqpcr <- myData[!is.na(myData$OPG) &
                          !is.na(myData$delta_ct_MminusE),]
...
```{r opgqpcr_3.75, echo=FALSE, warning=FALSE}
ggplot(myData[!is.na(myData$delta_ct_MminusE),],
       aes(x = delta_ct_MminusE, y = OPG)) +
  geom_point(aes(fill = qPCRsummary),
             alpha = .5, size = 4, pch = 21) +
...
data2 <- data1[data1$OPG > 0 & data1$delta_ct_MminusE > -THRESHOLD , ]
summary(lm(data2$OPG ~ data2$delta_ct_MminusE))
```

## OPG-qPCR-PCR

```{r opgpcrpcr_3.75, echo=FALSE, warning=FALSE}
# to compare, keep only samples tested for the 3 methods
completeData <- myData[!is.na(myData$OPG) &
                                   !is.na(myData$PCRstatus) &
                                   !is.na(myData$delta_ct_MminusE),]
...
```{r venn2_3.75, echo=FALSE, warning=FALSE, message=FALSE, fig.width=5,fig.height=3,fig.cap="\\label{fig:venn1}Comparison of detection: PCR vs flotation vs qPCR"}
myVennDiagram3(completeData)
```
...
```{r BCI_3.75, echo = FALSE, warning=FALSE}
ggplot(myData, aes(x = HI, y = BCI, fill = myData$OPG + 1)) +
  geom_point(pch = 21, size = 4, col = "black") +
  scale_fill_gradient(low = "white", high = "black", 
                      breaks = c(1, 10, 100, 1000, 10000, 100000, 1000000), 
...
# positive for flotation and have an hybrid index
N_qPCRPositive <- nrow(myDataHI[myDataHI$delta_ct_MminusE > - THRESHOLD &
                                  !is.na(myDataHI$delta_ct_MminusE), ])
totalqPCR <- nrow(myDataHI[!is.na(myDataHI$delta_ct_MminusE),])

# positive for flotation and have an hybrid index
N_QuantitativePositive <- nrow(myDataHI[(myDataHI$OPG > 0 | 
                                           myDataHI$delta_ct_MminusE > - THRESHOLD) &
                                          (!is.na(myDataHI$delta_ct_MminusE) |
                                          !is.na(myDataHI$OPG)),])
