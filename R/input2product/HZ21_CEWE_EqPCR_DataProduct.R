...
df <- read.csv('https://raw.githubusercontent.com/derele/Mouse_Eimeria_Field/master/data_products/qPCR_CEWE_eimeria_21_22.csv')
...
colnames(CEWE_Clean_M)[colnames(CEWE_Clean_M) %in% c( "Well.Position_Mus", "Mouse_ID_Mus", 'Target_Mus', "Task_Mus", 'plate_Mus')]       <- c("Well.Position", "Mouse_ID", 'Target', "Task", 'plate')
# add Ct_index for pivot
CEWE_Clean_M <- CEWE_Clean_M %>% mutate(Ct_index = case_when (Well.Position %in% c('A4', 'B4', 'C4', 'D4', 'E4', 'F4', 'G4', 'H4',
                        ...
                        Target = case_when(Target == 'Mus'   ~ 'Mus',
                                           ...
                                           Target == 'CDC42' ~ 'CDC42',
                                           Target == 'COI'   ~ 'COI'))
CEWE_Clean_M <- pivot_wider(CEWE_Clean_M, names_from = "Ct_index", values_from = 'Ct_Mus') %>% arrange(Mouse_ID) %>% group_by(Mouse_ID) %>% 
...
colnames(CEWE_Clean_E)[colnames(CEWE_Clean_E) %in% c( "Well.Position_Eim", "Mouse_ID_Eim", 'Target_Eim', "Task_Eim", 'plate_Eim')]       <- c("Well.Position", "Mouse_ID", 'Target', "Task", 'plate')
# add Ct_index for pivot
CEWE_Clean_E <- CEWE_Clean_E %>% mutate(Ct_index = case_when (Well.Position %in% c('A1', 'B1', 'C1', 'D1', 'E1', 'F1', 'G1', 'H1',
                        ...
                        Target = case_when(Target == 'Mus'   ~ 'Eim',
                                           Target == 'Eim'   ~ 'Eim',
                                           Target == 'COI'   ~ 'COI'))
CEWE_Clean_E <- pivot_wider(CEWE_Clean_E, names_from = "Ct_index", values_from = 'Ct_Eim') %>% arrange(Mouse_ID) %>% group_by(Mouse_ID) %>% 
  fill(c(everything()), .direction = "downup") %>% ungroup() %>% distinct(Mouse_ID, .keep_all = T) %>% filter(Task != 'NTC')
...
CEWE_Clean <- CEWE_Clean %>% arrange(Mouse_ID) %>% group_by(Mouse_ID) %>% 
  fill(c(everything()), .direction = "downup") %>% ungroup() %>% distinct(Mouse_ID, .keep_all = T) %>% filter(Task != 'NTC')
rm(CEWE_Clean_M)
rm(CEWE_Clean_E)
...
CEWE_Clean <- CEWE_Clean %>% 
  mutate(delta_ct_cewe_MminusE = (Ct_Mean_Mus - Ct_Mean_Eim)) %>%
...
# STANDARD CURVE
SC <- df %>% filter(Task %in% c('STANDARD'))
SC_1 <- SC %>% filter(plate == 'qPCR_eimeria_field_CEWE_JK_04032022.eds')
SC_1$Mouse_ID <- gsub(pattern = "V10_*", replacement = "SC1_V10_", x = SC_1$Mouse_ID)
SC_2 <- SC %>% filter(plate == 'qPCR_eimeria_field_CEWE_JK_04032022_2.eds')
SC_2$Mouse_ID <- gsub(pattern = "V10_*", replacement = "SC2_V10_", x = SC_2$Mouse_ID)
SC <- full_join(SC_1, SC_2)
rm(SC_1)
rm(SC_2)
colnames(SC) <- paste(colnames(SC), 'Eim', sep = '_')
colnames(SC)[colnames(SC) %in% c( "Well.Position_Eim", "Mouse_ID_Eim", 'Target_Eim', "Task_Eim", 'plate_Eim')]       <- c("Well.Position", "Mouse_ID", 'Target', "Task", 'plate')
...
SC <- pivot_wider(SC, names_from = "Ct_index", values_from = 'Ct_Eim') %>% arrange(Mouse_ID) %>% group_by(Mouse_ID) %>% fill(c(everything()), .direction = "downup") %>% ungroup() %>% distinct(Mouse_ID, .keep_all = T)

# write STANDARD CUFVE data frame in a csv file
write.csv(SC, "~/Documents/Mouse_Eimeria_Field/data_products/HZ21_CEWE_EqPCR_SC.csv", row.names=FALSE)
